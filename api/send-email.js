...
<body style="margin:0; padding:0; background:#f5f0e8; font-family: Arial, sans-serif;">
  ...
  if (type === 'postcard' && postcards?.length) {
    for (let i = 0; i < postcards.length; i++) {
      const { frontUrl, backUrl, backWithQr, denomination } = postcards[i]
      ...
      const backSource = backWithQr || backUrl
      if (backSource) {
        if (backWithQr) {
          // backWithQr is already base64 data URL
          const base64Data = backWithQr.split(',')[1]
          attachments.push({ filename: `${label}-back.png`, content: base64Data })
        } else {
